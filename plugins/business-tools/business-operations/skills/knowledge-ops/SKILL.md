---
name: knowledge-ops
description: Use when a Head of Ops, Knowledge Manager, or internal TPM needs to author, validate, or clean up company SOPs and internal runbooks (procurement intake, vendor offboarding, incident-comms cascade, employee onboarding, system-access provisioning) — including 5W2H completeness checks (Who-What-When-Where-Why-How-HowMuch), cross-link and orphan-page validation across a Notion/Confluence/Obsidian wiki, KB ingestion + hygiene reporting, and runbook step verification (named owner, expected duration, success signal, rollback path, escalation contact). Trigger with phrases like "write an SOP", "author a runbook", "5W2H check", "KB hygiene", "wiki cleanup", or "ops onboarding docs". Pairs Ishikawa's 5W2H, Gawande's Checklist Manifesto, ISO 9001, ITIL v4, and the Google SRE Workbook with deterministic stdlib-only Python tools that score completeness, detect anti-patterns, and emit prioritized cleanup lists. Distinct from personal PKM, system-ops debugging runbooks, and BPMN process design (see process-mapper).
context: fork
