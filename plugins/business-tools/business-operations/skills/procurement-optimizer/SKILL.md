...
description: Use when running an annual SaaS audit, doing a category-level spend review, or rationalizing the supplier base — spend categorization (UNSPSC-aligned), purchasing-cycle analysis, or risk-balanced supplier consolidation. Trigger with phrases like "spend audit", "SaaS audit", "spend categorization", "supplier rationalization", "supplier consolidation", "purchasing cycle", "category strategy", or "duplicate SaaS". Ships 3 stdlib-only Python tools (UNSPSC-aligned spend categorizer with Pareto breakdown and industry profiles, purchasing-cycle analyzer that surfaces bottleneck categories per Goldratt's Theory of Constraints, supplier-consolidation planner that refuses single-source recommendations for tier-1 categories without a documented break-glass plan), reference docs citing authoritative sources (A.T. Kearney / Hackett / Spend Matters / UNSPSC / ISM), and a spend-intake template. Distinct from vendor-management (performance scoring), financial-analysis (close + report), and contract-law advisory.
